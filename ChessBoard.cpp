...
bool Chessboard::select(int fromRow, int fromCol) 
{
    ...
    {
        cout << "Bląd: brak figury na pozycji poczatkowej." << endl;
        return false;
    }
...
void Chessboard::displayMoves()
{
    ...
    for (int i = 0; i < coords.size(); i++)
    {
        ...
        std::cout << char(coords[i].col + 65) << 8 - coords[i].row << std::endl; // wyswietlanie mozliwych ruchow
    }
}
...
void Chessboard::makeMove(int fromRow, int fromCol, int toRow, int toCol)
{
    ...
    {
        cout << "Blad: Nieprawidlowy ruch." << endl;
        return;
    }

    if (fromRow == toRow && fromCol == toCol)
    {
        cout << "Blad: Nieprawidlowy ruch.. Figura musi się przemieszczac." << endl;
        return;
    }
    ...
    if (board[toRow][toCol]->getSymbol() == 'P' and (toRow == 0 or toRow == 7)) // awans pionka
    {
        if (board[toRow][toCol]->getColor() == 'w')
        {
            ...
            {
                char aw;
                std::cout << "Wybierz figure do awansu: ";
                std::cin >> aw;
...
bool Chessboard::isKingInCheck() 
{
    ...
    for (int i = 0; i < coords.size(); i++) // jeżeli pozucja krola znajduje się w tablicy możliwych ruchow przeciwnika wyświetl komunikat
    {
        ...
        {
            std::cout << "(Uwaga!!! Bialy w szachu)" << std::endl;
        }

        if (coords[i].col == BK_col and coords[i].row == BK_row)
        {
            std::cout << "(Uwaga!!! Czarny w szachu)" << std::endl;
        }
...
{
    cout << (playerTurn ? "Bialy" : "Czarny") << ", wybierz bierke (np. A2): ";
}

void Chessboard::displayMessage(std::string str)
{
    std::cout << str;
}
...
{
    cout << (playerTurn ? "Czarny" : "Bialy") << " wygrywa!" << endl;
}
